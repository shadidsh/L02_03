...
public class DbUserTest {
	...
	@Test
	public void testRemoveNonExistingProf() {		
		assertEquals(-1, dbUser.removeUser("nonexistent"));
	}
	
	public void getUserByCorrectLoginAndUsername() {
		UserLogin user = dbUser.getUser("bestProf", "userIsTrue");
		UserLogin user1 = dbUser.getUser("bestProf");
		assertEquals(user.getId(), prof.getId());
		assertEquals(user1.getId(), prof.getId());
	}
