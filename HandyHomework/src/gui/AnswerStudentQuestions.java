...
public class AnswerStudentQuestions extends JFrame {
		...
		listQuestions.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if (e.getClickCount() == 2 && qName != tq.getName()) {
					...
					for (int i = 0; i < textQuests.size(); i++) {
						if (textQuests.get(i).getName() == qName) {
						}
					}
					//dasdasd  use aid (assignment id) to get list of all questions and 
					// go to that question by looking at the index of the question in the 
					// display and setting that to the .next()
	...
	/*
	public void render(String questionContent,JFrame frame) {
